...
issuer IDs) are stored as one-way privacy hashes. Content is represented only
by its cryptographic hash, never by its plaintext.
...
| `duration_ms`        | integer or null   | full       | Verification duration in milliseconds.                             |
...
| `duration_ms`        | number            | Verification duration in milliseconds.                             |
...
2. **Link**: Set `previous_hash` to `P`. For the genesis entry (first entry
   in the chain), set `previous_hash` to the empty string `""`.
3. **Serialize**: JSON-serialize the extracted fields with sorted keys and no
   whitespace separators (`separators=(",", ":")`). Non-string values are
   converted via their default string representation.
...
- `chain_position`: 1
- `previous_hash`: `null` (serialized as empty string for hashing)
...
function compute_entry_hash(entry, previous_hash):
    ...
    canonical = json_serialize(hashable, sort_keys=true, separators=(",",":"))
...
function privacy_hash(value):
    full_hash = sha256(utf8_encode(value)).hex_digest()
    return "sha256:" + full_hash[0:32]
```

This produces a 128-bit (32 hex character) truncated SHA-256 hash prefixed with
`"sha256:"`. The truncation provides sufficient collision resistance for audit
purposes while reducing storage requirements. The `sha256:` prefix indicates the
algorithm used, supporting future algorithm migration.
...
4. Uses the privacy hash function from Section 6 for all identifier fields.
